...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ========================================================================
  // 9. Cross-Viewport Synchronization System (Desktop <-> Mobile)
  // ========================================================================
  const urlParams = new URLSearchParams(window.location.search);
  const frameId = urlParams.get('frame') || (window.innerWidth > 768 ? 'desktop-frame' : 'mobile-frame');

  function broadcastSync(payload) {
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ ...payload, sourceId: frameId }, '*');
    }
  }
  ...
  window.addEventListener('scroll', () => {
    ...
    broadcastSync({ type: 'SYNC_SCROLL', ratio });
  }, { passive: true });

  // B. Synchronize Input, Mode, and Templates
  if (promptTextarea) {
    promptTextarea.addEventListener('input', () => {
      broadcastSync({ type: 'SYNC_INPUT', value: promptTextarea.value });
    });
  }
  ...
  modePills.forEach(pill => {
    pill.addEventListener('click', () => {
      const mode = pill.getAttribute('data-mode');
      if (mode) {
        broadcastSync({ type: 'SYNC_MODE', mode });
      }
    });
  });
  ...
  templatePills.forEach(pill => {
    pill.addEventListener('click', () => {
      const templateKey = pill.getAttribute('data-template');
      if (templateKey) {
        broadcastSync({ type: 'SYNC_TEMPLATE', templateKey });
      }
    });
  });
  ...
  if (chatToggleBtn) {
    chatToggleBtn.addEventListener('click', () => {
      const isOpen = chatDrawer ? chatDrawer.classList.contains('open') : false;
      broadcastSync({ type: 'SYNC_CHAT_TOGGLE', isOpen });
    });
  }

  if (chatCloseBtn) {
    chatCloseBtn.addEventListener('click', () => {
      broadcastSync({ type: 'SYNC_CHAT_TOGGLE', isOpen: false });
    });
  }

  // C. Listen for Sync Messages from Parent Workbench
  window.addEventListener('message', (event) => {
    const data = event.data;
    if (!data || data.sourceId === frameId) return;

    if (data.type === 'SYNC_SCROLL') {
      isReceivingScroll = true;
      const maxScroll = document.documentElement.scrollHeight - window.innerHeight;
      const targetY = data.ratio * maxScroll;
      window.scrollTo({
        top: targetY,
        behavior: 'auto'
      });
    ...
    else if (data.type === 'SYNC_INPUT') {
      if (promptTextarea) {
        promptTextarea.value = data.value;
        isUserTyping = data.value.length > 0;
        promptTextarea.style.height = 'auto';
        promptTextarea.style.height = promptTextarea.scrollHeight + 'px';
      }
    }
    else if (data.type === 'SYNC_MODE') {
    ...
    else if (data.type === 'SYNC_TEMPLATE') {
      if (templatePrompts[data.templateKey] && promptTextarea) {
        promptTextarea.value = templatePrompts[data.templateKey];
        isUserTyping = true;
        promptTextarea.style.height = 'auto';
        promptTextarea.style.height = promptTextarea.scrollHeight + 'px';
      }
    }
    else if (data.type === 'SYNC_CHAT_TOGGLE') {
      if (chatDrawer) {
        if (data.isOpen) {
    ...
    else if (data.type === 'SYNC_CHAT_MESSAGE') {
      if (chatMessages) {
        const userMsg = document.createElement('div');
        userMsg.className = 'chat-bubble user';
        userMsg.textContent = data.text;
        chatMessages.appendChild(userMsg);
        chatMessages.scrollTop = chatMessages.scrollHeight;
